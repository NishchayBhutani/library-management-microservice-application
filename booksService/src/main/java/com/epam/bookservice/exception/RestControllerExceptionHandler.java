...
public class RestControllerExceptionHandler {
	...
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	public ExceptionResponse handleBookException(BookException ex, WebRequest req) {
	...
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	public ExceptionResponse handleRuntimeException(DataIntegrityViolationException ex, WebRequest req) {
		ExceptionResponse exceptionResponse = new ExceptionResponse(new Date().toString(),
				HttpStatus.BAD_REQUEST.name(), ex.getMessage(), req.getDescription(false));
		log.error(ex.getMessage());
		return exceptionResponse;
